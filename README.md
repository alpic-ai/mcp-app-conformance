...
This platform certifies **hosts**, so every test is a host test — IDs carry no
actor prefix and are namespaced by the spec **capability area** (WPT-path style).

Below is every normative requirement the [`2026-01-26` spec](https://github.com/modelcontextprotocol/ext-apps/blob/main/specification/2026-01-26/apps.mdx)
...
- **Vantage** = where the requirement can actually be observed: `in-view` (from inside the iframe — this harness), `server` (only the test server sees it), `agent` (needs the model's view / a multi-turn conversation), `transport` (sandbox-internal, not forwarded to the view), `manual` (host-internal / UX side effect, not auto-measurable). ⚠️ flags a measurement caveat.
