...
import { AssertionError, captureHostSignals, SkipError, type HostSignals } from "./assert";
...
export class SuiteEngine implements RequestPump {
  ...
  async start(filter?: { manual?: boolean; id?: string } | null): Promise<void> {
    if (!this.app || !this.signals) throw new Error("engine not attached to an app");
  ...
  private async runOne(def: TestDef): Promise<SubtestResult> {
    ...
    try {
      ...
      if (err instanceof SkipError) {
        status = "SKIP";
      } else if (err instanceof AssertionError && /timed out/.test(err.message)) {
        status = "TIMEOUT";
      } else {
        status = "FAIL";
      }
