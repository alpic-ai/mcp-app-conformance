...
const toRow = (r: SubtestResult): Row => ({
  id: r.id, name: r.name, status: r.status, clause: r.clause, vantage: r.vantage, manual: r.manual, caveat: r.caveat, message: r.message,
});

/** A pending interaction request plus the resolver that settles the test. */
type PendingInteraction = { req: InteractionRequest; resolve: (v: boolean) => void };
...
function ConformanceRunner() {
  ...
  const run = useCallback(async () => {
    ...
    const results = await runAll(app, signalsRef.current ?? undefined, {
      ...
      onEnterManual: async () => {
        try { await app.requestDisplayMode({ mode: "fullscreen" }); } catch { /* host may decline */ }
      },
  ...
  const runTrigger = useCallback((req: InteractionRequest) => {
    void Promise.resolve(req.trigger?.run()).catch((e) => console.error("[conformance] trigger error:", e));
  }, []);
    ...
    <main className="wrap">
      <header className="head">
        ...
        <div className="head-actions">
          ...
          <button className="run-btn" onClick={run} disabled={!app || running}>
            {running ? `Running ${done}/${rows.length}…` : ran ? "Re-run tests" : "Run conformance tests"}
          </button>
        </div>
      </header>
      ...
      {running && (
        <div className="progress" role="progressbar" aria-valuenow={done} aria-valuemax={rows.length}>
          <div className="progress-bar" style={{ width: `${rows.length ? (done / rows.length) * 100 : 0}%` }} />
        </div>
      )}
      ...
      <table className="grid">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className={r.id === runningId ? "running" : undefined}>
              ...
              <td>
                {r.id === runningId
                  ? <span className="st st-running"><span className="spinner" />running…</span>
          ...
          <div className="interaction-card">
            ...
            <div className="interaction-actions">
              {interaction.req.trigger && (
                <button className="trigger-btn" onClick={() => runTrigger(interaction.req)}>
              ...
              {interaction.req.kind === "ack" ? (
                ...
                <>
                  ...
                  <button className="verdict-btn no" onClick={() => interaction.resolve(false)}>❌ It didn’t</button>
                </>
              )}
            </div>
          </div>
